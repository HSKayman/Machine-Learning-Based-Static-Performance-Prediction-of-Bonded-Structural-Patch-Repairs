...
#
# The analysis is run for the ANN (this study's primary model) and for the best
# overall model from the comparison. Native tree importances are also aggregated
# to the original features when available.
...
def permutation_importance(model, pre, real_df, feature_cols, target_col,
                           n_repeats=50, seed=42) -> Dict[str, float]:
    # Measure Δ MAPE when each original feature column is permuted.
    X, y = pre.transform(real_df)
...
def run_dataset(dataset_type: str, config: Dict) -> None:
    # Compute and save permutation importance for ANN and the best overall model.
    ...
    target = syn_cfg.get("target_samples", 300)
    ann_def = config["training"].get("ann", {})
    kan_def = config["training"].get("kan", {})
    rs = config.get("evaluation", {}).get("random_state", 42)

    # Deployed model: augment full real baseline -> train.
    aug, _ = augment_dataframe(baseline, target, syn_cfg)
    ...
    models_to_explain = []
    for name in dict.fromkeys(["ANN", best_overall]):  # unique, keep order
        if name in best_configs:
            models_to_explain.append(name)
    ...
    for name in models_to_explain:
        ...
        imp = permutation_importance(model, pre, baseline, feature_cols, target_col)
        total = sum(abs(v) for v in imp.values()) or 1.0
        print(f"\n  {name} (permutation importance, real baseline):")
        for col, v in sorted(imp.items(), key=lambda kv: -kv[1]):
            print(f"    {col:28s} dMAPE={v:6.3f}%   ({100 * v / total:5.1f}%)")
            rows.append({"model": name, "feature": col, "delta_mape": v,
                         "relative_pct": 100 * v / total})
